...
namespace CR.WorkingDayService.StringSource
{
    ...
    public class StringWorkingDaySource<T> : IWorkingDaySource
    {
        ...
        /// </summary>
        /// <param name="url">The url of the state from.</param>
        /// <param name="parseAction">The action to get the state from the url content.</param>
        /// <param name="checkAction">The action to get whether a DateTime is on a Working Day based on the current state.</param>
        public StringWorkingDaySource(string url, Func<string, T> parseAction, Func<DateTime, T, bool> checkAction)
            : this(parseAction, checkAction) => State = parseAction(url);
